...
sleep 10
#MySQL, RabbitMQ, NTP
echo apt-get install -y mysql-server python-mysqldb rabbitmq-server ntp
sleep 10
...
sed -i 's|connection = sqlite:////var/lib/keystone/keystone.db|connection = mysql://keystone:openstacktest@192.168.0.84/keystone |g' /etc/keystone/keystone.conf
...
export OS_AUTH_URL=http://192.168.0.84:5000/v2.0
...
sed -i 's|sql_connection = sqlite:////var/lib/glance/glance.sqlite|sql_connection = mysql://glance:openstacktest@192.168.0.84/glance|g' $GlanceAPIConf $GlanceRegConf
sed -i 's|auth_host = 127.0.0.1|auth_host = 192.168.0.84|g' $GlanceAPIConf $GlanceRegConf
...
sed -i 's|\[filter:authtoken]|[filter:authtoken]\nauth_host = 192.168.0.84|' $GlanceRegPaste $GlanceAPIPaste
...
sed -i 's|connection = sqlite:////var/lib/neutron/neutron.sqlite|connection = mysql://neutron:openstacktest@192.168.0.84/neutron|g' $NeutronConf
...
sed -i 's|auth_url = http://localhost:5000/v2.0|auth_url = http://192.168.0.84:35357/v2.0|g' $MetaDataAgent
...
sed -i 's|# nova_metadata_ip = 127.0.0.1|nova_metadata_ip = 192.168.0.84|g' $MetaDataAgent
...
auth_url = http://192.168.0.84:35357/v2.0
...
admin_tenant_name = service
admin_user = neutron
admin_password = openstacktestauth_url = http://192.168.0.84:35357/v2.0
...
local_ip = 192.168.0.84
[securitygroup]
firewall_driver = neutron.agent.linux.iptables_firewall.OVSHybridIptablesFirewallDriver
[database]
sql_connection=mysql://neutron:openstacktest@192.168.0.84/neutron
EOF
...
nova_url=http://192.168.0.84:8774/v1.1/
...
# Imaging service
glance_api_servers=192.168.0.84:9292
image_service=nova.image.glance.GlanceImageService

# Vnc configuration
novnc_enabled=true
novncproxy_base_url=http://192.168.0.84:6080/vnc_auto.html
novncproxy_port=6080
vncserver_proxyclient_address=192.168.0.84
vncserver_listen=0.0.0.0

# Network settings
network_api_class=nova.network.neutronv2.api.API
neutron_url=http://192.168.0.84:9696
neutron_auth_strategy=keystone
neutron_admin_tenant_name=service
neutron_admin_username=neutron
neutron_admin_password=openstacktest
neutron_admin_auth_url=http://192.168.0.84:35357/v2.0
...
metadata_host = 192.168.0.84
metadata_listen = 192.168.0.84
...
sed -i 's|auth_host = 127.0.0.1|auth_host = 192.168.0.84|g' $NovaAPIPaste
...
sql_connection = mysql://cinder:openstacktest@192.168.0.84/cinder
...
sed -i 's|\[filter:authtoken]|[filter:authtoken]\nservice_host = 192.168.0.84|g' $CinderAPIPaste
sed -i 's|\[filter:authtoken]|[filter:authtoken]\nservice_port = 5000|g' $CinderAPIPaste
sed -i 's|\[filter:authtoken]|[filter:authtoken]\nauth_host = 192.168.0.84|g' $CinderAPIPaste
...
swift-ring-builder account.builder add z1-192.168.0.84:6002/sda4 100
swift-ring-builder container.builder add z1-192.168.0.83:6001/sda4 100
swift-ring-builder object.builder add z1-192.168.0.84:6000/sda4 100
...
sed -i 's|\[DEFAULT]|[DEFAULT]\nsql_connection = mysql://heat:openstacktest@192.168.0.84/heat|g' $HeatConf
cat >>  $HeatConf << EOF
[keystone_authtoken]
auth_host = 192.168.0.84
auth_port = 35357
auth_protocol = http
auth_uri = http://192.168.0.84:5000/v2.0
admin_tenant_name = service
admin_user = heat
admin_password = openstacktest
...
[ec2_authtoken]
auth_uri = http://192.168.0.84:5000/v2.0
keystone_ec2_uri = http://192.168.0.84:5000/v2.0/ec2tokens
EOF
